...
def print_tree(dir_path, indent="", level=0, max_depth=None, max_files=None, is_last=False):
    ...
    items = sorted(items)  # Sorted to ensure consistent output
    ...
    for i, folder in enumerate(dirs):
        ...
        print_tree(full_path, next_indent, level + 1, max_depth, max_files, i == len(dirs) - 1 and len(files) == 0)

    # Display up to `max_files` files
    ...
    # If there are hidden files, print a message in red
    ...
    # Add an extra empty line (without extra '|') to visually separate directories and files
...
if __name__ == "__main__":
    ...
    parser.add_argument("max_depth", type=int, nargs='?', default=0,
                        help="The maximum depth of directories to display. Use 0 for unlimited.")
    parser.add_argument("max_files", type=int, nargs='?', default=None,
                        help="The maximum number of files to display in each directory. Directories are not limited.")
    ...
    print_tree(directory, max_depth=max_depth, max_files=max_files)
